...
2. **Position: immediately next to the "Prototype" app title, on the left side of TopNav** — not grouped with the auth/comment-toggle controls on the right. This is a fixed placement decision, not left to per-run judgment; if a previous run put it somewhere else, move it here rather than leaving it where it is.
